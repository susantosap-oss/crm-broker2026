    ...
    <div style="padding:14px">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between">
        <div>
          ...
          ${korNama ? `<p style="color:rgba(212,168,83,0.7);font-size:10px;margin:3px 0 0"><i class="fa-solid fa-user-tie" style="font-size:9px;margin-right:3px"></i>${escHtml(korNama)}</p>` : ''}
        </div>
...
function renderKoordinatorList(candidates, project, slot) {
  ...
  listEl.innerHTML = candidates.map(c => {
    ...
    return `<div onclick="saveKoordinatorSlot('${c.id}', '${escHtml(c.nama)}')"
      ...
      <div>
        <p style="color:${isActive ? '#D4A853' : '#fff'};font-size:13px;font-weight:600;margin:0">${escHtml(c.nama)}</p>
        <p style="color:rgba(255,255,255,0.35);font-size:11px;margin:2px 0 0">${escHtml(c.nama_kantor || '')} · ${c.role}</p>
      </div>
    ...
    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;overflow:hidden">
      ${projects.slice(0, 5).map((p, i) => {
        ...
        return `<div onclick="navigateTo('primary')" style="display:flex;align-items:center;gap:12px;padding:12px 14px;${i < Math.min(projects.length,5)-1 ? 'border-bottom:1px solid rgba(255,255,255,0.05)' : ''};cursor:pointer"
          ...
          <div style="flex:1;min-width:0">
            <p style="color:#fff;font-size:13px;font-weight:600;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(p.Nama_Proyek)}</p>
            <p style="color:rgba(255,255,255,0.35);font-size:11px;margin:2px 0 0">${escHtml(p.Nama_Developer)} · ${escHtml(p.Harga_Format || 'On Request')}</p>
          </div>
      ...
      ${korList.map((k, i) => `
        <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;${i < korList.length-1 ? 'border-bottom:1px solid rgba(255,255,255,0.04)' : ''}">
          <div style="width:30px;height:30px;border-radius:8px;background:rgba(212,168,83,0.12);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#D4A853">
            ${escHtml((k.nama||'?')[0].toUpperCase())}
          </div>
          <span style="flex:1;font-size:13px;color:#fff;font-weight:500">${escHtml(k.nama)}</span>
    ...
    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;overflow:hidden">
      ...
      ${projects.slice(0, 4).map((p, i) => {
        ...
        return `<div onclick="navigateTo('primary')" style="display:flex;align-items:center;gap:10px;padding:11px 14px;${i < Math.min(projects.length,4)-1 ? 'border-bottom:1px solid rgba(255,255,255,0.04)' : ''};cursor:pointer"
          ...
          <div style="flex:1;min-width:0">
            <p style="color:#fff;font-size:13px;font-weight:600;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(p.Nama_Proyek)}</p>
            <p style="color:rgba(255,255,255,0.35);font-size:11px;margin:2px 0 0"><i class="fa-solid fa-user-tie" style="font-size:9px;margin-right:3px"></i>${escHtml(kor)}</p>
          </div>
