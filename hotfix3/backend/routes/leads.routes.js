...
router.get('/', async (req, res) => {
  try {
    ...
    if (role === 'agen' || role === 'koordinator') {
      ...
      // Admin tidak diijinkan melihat leads
      return res.status(403).json({ success: false, message: 'Akses ditolak. Role Admin tidak dapat melihat data leads.' });
    } else if (role === 'business_manager') {
      // BM lihat leads anggota timnya saja
      // Jika team_id ada di JWT → filter by team
      // Jika tidak ada → fallback ke leads sendiri saja (bukan semua)
      if (team_id) {
        leads = leads.filter(l => l.Team_ID === team_id || l.Agen_ID === id);
      } else {
        // Fallback: cari anggota tim BM ini dari sheet AGENTS
        try {
          const agentRows = await sheetsService.getRange(SHEETS.AGENTS);
          const { COLUMNS } = require('../config/sheets.config');
          const toAgent = (row) => COLUMNS.AGENTS.reduce((o, col, i) => { o[col] = row[i] || ''; return o; }, {});
          const agentData = agentRows && agentRows.length > 1 ? agentRows.slice(1) : [];
          // Cari BM ini di sheet untuk dapat Team_ID-nya
          const meRow = agentData.find(r => toAgent(r).ID === id);
          const myTeamId = meRow ? toAgent(meRow).Team_ID : '';
          if (myTeamId) {
            leads = leads.filter(l => l.Team_ID === myTeamId || l.Agen_ID === id);
          } else {
            leads = leads.filter(l => l.Agen_ID === id);
          }
        } catch (_) {
